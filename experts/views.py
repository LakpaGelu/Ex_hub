...
def quick_sort(arr):
    ...
    for expert in arr:
        if expert.rating < pivot:
            smaller.append(expert)
        elif expert.rating == pivot:
            equal.append(expert)
        else:
            larger.append(expert)

    return quick_sort(larger) + equal + quick_sort(smaller)

# Popularity based ranking algorithm (based on ratings)

def retrieve_experts(request):
    experts_object = Expert.objects.all()

    # Store experts in a list
    experts_list = list(experts_object)

    # Sort experts using Quick Sort based on ratings
    experts_list = quick_sort(experts_list)
